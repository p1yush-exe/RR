...
def process_candidates(input_path, output_path):
    ...
    for count, candidate in enumerate(iter_candidates(input_path), 1):
        ...
        # Check for research-only candidates and drop them in Phase 1
        career = candidate.get("career_history", [])
        all_titles_desc = " ".join(
            [c.get("title", "") for c in career] + [c.get("description", "") for c in career]
        ).lower()
        ...
        if len(top_2000) < 2000:
            heapq.heappush(top_2000, (final, candidate.get("candidate_id", ""), memo))
        else:
            heapq.heappushpop(top_2000, (final, candidate.get("candidate_id", ""), memo))

    # Sort top 2000 descending by final_score, breaking ties with candidate_id
    top_2000_sorted = sorted(top_2000, key=lambda x: (-x[0], x[1]))
    ...
    if os.path.exists(model_path):
        try:
            ...
            features = [xgb_features_from_memo(item[2]) for item in top_2000_sorted]
            ...
            reranked = []
            for i in range(len(top_2000_sorted)):
                reranked.append((float(preds[i]), top_2000_sorted[i][1], top_2000_sorted[i][2]))
            reranked.sort(key=lambda x: (-x[0], x[1]))
            top_2000_sorted = reranked
        except Exception:
            pass  # Fall back to heuristic scoring

    top_100 = sorted(top_2000_sorted[:100], key=lambda x: (-output_score(x[0]), x[1]))

    from reasoning import generate_reasoning
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["candidate_id", "rank", "score", "reasoning"])
        for i, (score, cid, memo) in enumerate(top_100, 1):
