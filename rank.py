...
def process_candidates(input_path, output_path):
    ...
    with open_func(input_path, 'rt', encoding='utf-8') as f:
        for line in f:
            ...
            # top matched skills
            top_skills = [s.get("name") for s in skills if s.get("name", "").lower() in RELEVANT_SKILLS][:3]
            ...
            is_big_tech_lifer = num_roles == 1 and career and career[0].get("duration_months", 0) >= 72 and any(bt in career[0].get("company", "").lower() for bt in BIG_TECH)
            
            if is_job_hopper or is_big_tech_lifer:
                yoe_note = None
                
            all_titles_desc = " ".join([c.get("title", "") for c in career] + [c.get("description", "") for c in career]).lower()
            ...
            if "vector" not in all_titles_desc and not any("vector" in s.get("name", "").lower() or "pinecone" in s.get("name", "").lower() or "faiss" in s.get("name", "").lower() for s in skills):
                concerns.append("no vector DB experience")
            
            positive_signals = []
            if signals.get("recruiter_response_rate", 0) > 0.8:
                positive_signals.append(f"response rate {int(signals.get('recruiter_response_rate', 0)*100)}%")
    ...
    if os.path.exists(model_path):
        try:
            ...
            pass # Fall back to heuristic scoring if xgboost is not installed
            
    top_100 = top_1000_sorted[:100]
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for rank, item in enumerate(top_100, 1):
            ...
            writer.writerow([cid, rank, f"{score:.4f}", reasoning_str])
